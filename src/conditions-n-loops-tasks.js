...
function convertToRomanNumerals(num) {
  ...
  if (numberDecimal > 0) {
    for (let index = 0; index < romanNumbers.length; index += 1) {
      while (numberDecimal >= romanNumbers[index][1]) {
        result += romanNumbers[index][0];
        numberDecimal -= romanNumbers[index][1];
      }
    }
  }
  return result;
...
function getNearestBigger(number) {
  ...
  let prevIndex = digits.length - 2;
  while (prevIndex >= 0 && digits[prevIndex] >= digits[prevIndex + 1]) {
    prevIndex -= 1;
  }

  if (prevIndex < 0) return number;

  let lastIndex = digits.length - 1;
  while (digits[lastIndex] <= digits[prevIndex]) {
    lastIndex -= 1;
  }

  [digits[prevIndex], digits[lastIndex]] = [
    digits[lastIndex],
    digits[prevIndex],
  ];
  ...
  const result = digits.reduce((acc, value) => {
    return acc * 10 + value;
  }, 0);
